...
def render_anime_card(spot):
    ...
    """
    img_src = spot.get('image') or spot.get('img') or "https://via.placeholder.com/400x225.png?text=No+Signal"
    ...
    city = spot.get('_city') or "Coordinates"
    lat, lon = spot.get('lat', 0), spot.get('lon', 0)
    
    # Generate map link
    map_url = f"https://www.amap.com/search?query={lat},{lon}"
...
<div class="anime-card fade-in">
    <div style="display: flex; gap: 24px; align-items: stretch;">
        <div style="flex: 2; min-width: 200px;" class="card-image-container">
            <img src="{img_src}" style="width: 100%; height: 100%; object-fit: cover; min-height: 160px; transition: transform 0.5s;">
        </div>
        <div style="flex: 3; display: flex; flex-direction: column; justify-content: space-between; padding-top: 5px; padding-bottom: 5px;">
            ...
            <div style="display: flex; gap: 12px; align-items: center; margin-top: auto;">
                <a href="{map_url}" target="_blank" class="nav-btn">
                    🗺️ Navigate
                </a>
            </div>
        </div>
...
<div class="agent-box fade-in">
    ...
    <div style="flex-grow: 1;">
        <div style="font-weight: 700; color: #7AF; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px;">System Status</div>
        <div style="color: #2D3436;">{message}</div>
    </div>
</div>
"""
